...
class SmtSolver(command: String, opts: Seq[String] = Seq.empty) {
  ...
  /** Runs the SMT solver */
  def run(smtFileName: String, outFileName: String, solver: SMTSolverRunner): Int = {
  ...
  protected def runProcess(args: Seq[String], logger: FileProcessLogger, solver: SMTSolverRunner): Int = {
 ...
 */
class Encoder(csp: CSP, solver: SMTSolverRunner, smtFileName: String) {
  var translator = new Translator()
  var sugarCSP = new sugar.csp.CSP()
  var converter = new sugar.converter.Converter(sugarCSP)
  def init {
    translator = new Translator()
    sugarCSP = new sugar.csp.CSP()
    converter = new sugar.converter.Converter(sugarCSP)
  }
  def commit {
    // csp.commit
    sugarCSP.commit
  ...
  def encode: Boolean = {
    ...
    solver.checkTimeout
     println("Converting")
    sugar.converter.Converter.INCREMENTAL_PROPAGATION = true
    // no_norm,no_reduce,no_decomp_alldiff,replace_args,no_hints
    sugar.converter.Converter.NORMALIZE_LINEARSUM = false
    sugar.converter.Converter.REDUCE_ARITY = false
    sugar.converter.Converter.DECOMPOSE_ALLDIFFERENT = false
    sugar.converter.Converter.REPLACE_ARGUMENTS = true
    sugar.converter.Converter.HINT_ALLDIFF_PIGEON = false
    converter.convert(expressions)
    solver.checkTimeout
    expressions.clear
    sugar.expression.Expression.clear
    // println("Propagating")
    sugarCSP.propagate
    solver.checkTimeout
    if (sugarCSP.isUnsatisfiable)
      ...
      // sugarCSP.simplify
      // solver.checkTimeout
      val outSmt = new sugar.OutputSMT()
  ...
  def encodeDelta {
    ...
    solver.checkTimeout
    sugar.converter.Converter.INCREMENTAL_PROPAGATION = false
    // no_norm,no_reduce,no_decomp_alldiff,replace_args,no_hints
    sugar.converter.Converter.NORMALIZE_LINEARSUM = false
    sugar.converter.Converter.REDUCE_ARITY = false
    sugar.converter.Converter.DECOMPOSE_ALLDIFFERENT = false
 ...
 */
class SMTSolverRunner(csp: CSP, var smtSolver: SmtSolver = Z3) extends AbstractSolver(csp) {
  val solverName = "smt"
  ...
  @deprecated("use encodeDelta method of [[copris.smt.Solver]] instead", "2.2.0")
 ...
 * Class for SMT DSL
 */
class SMT(val csp: CSP, var solver: SMTSolverRunner) extends CoprisTrait {
  def this(csp: CSP) =
    this(csp, new SMTSolverRunner(csp))
  def this(csp: CSP, smtSolver: SmtSolver) =
    this(csp, new SMTSolverRunner(csp, smtSolver = smtSolver))
  def this(smtSolver: SmtSolver) =
    this(CSP(), smtSolver = smtSolver)
  def this() =
    this(CSP())
  def use(newSolver: AbstractSolver): Unit = newSolver match {
    case sugarSolver: SMTSolverRunner =>
      solver = sugarSolver
  }
