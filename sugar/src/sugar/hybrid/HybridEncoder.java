...
import sugar.SugarConstants;
import sugar.SugarException;
import sugar.SugarMain;
...
import sugar.encoder.Encoding;
import sugar.encoder.Problem;
import sugar.pb.PBExpr;
import sugar.pb.PBProblem;
...
//          int c = (int) Math.ceil((double) b / a) - 1;
    ...
    private void encodeObjective() throws SugarException {
        if (HybridConverter.USE_PMINIMAL) {
            if (csp.getObjective() != CSP.Objective.MINIMIZE)
    ...
    public void encode() throws SugarException {
        ...
        if (csp.getObjective() != CSP.Objective.NONE) {
            encodeObjective();
        }
        for (IntegerVariable v : csp.getIntegerVariables()) {
